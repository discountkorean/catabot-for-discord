...
def build_help_pages(is_admin: bool) -> list[discord.Embed]:
    ...
    p2.add_field(
        name="status", value="Tracker state, poll interval, stores, and per-store channel overrides.", inline=False
    )
    p2.add_field(
        name="subscribe [store] [names] [sizes]",
        value=(
    ...
    p2.add_field(
        name="subscriptions [user]", value="List your active subscriptions with filters and IDs.", inline=False
    )
    ...
    p2.add_field(
        name="catalog [store]",
        value="Paginated product list — 🟢 full / 🟠 partial / 🔴 sold out — sorted newest first.",
        inline=False,
    )
    p2.add_field(
        name="search [query] [stores...]", value="Search for a product by name across up to 5 stores.", inline=False
    )
